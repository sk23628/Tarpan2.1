...
package icreativ.android.volley;

import icreativ.android.volley.mock.MockRequest;
import icreativ.android.volley.mock.ShadowSystemClock;
import icreativ.android.volley.toolbox.NoCache;
import icreativ.android.volley.utils.ImmediateResponseDelivery;
...
public class RequestQueueIntegrationTest {
    ...
    @Test public void add_requestProcessedInCorrectOrder() throws Exception {
        ...
        lowerPriorityReq.setPriority(Request.Priority.LOW);
        higherPriorityReq.setPriority(Request.Priority.HIGH);

        RequestQueue.RequestFinishedListener listener = mock(RequestQueue.RequestFinishedListener.class);
    ...
    @Test public void add_requestFinishedListenerSuccess() throws Exception {
        ...
        Request request = new MockRequest();
        RequestQueue.RequestFinishedListener listener = mock(RequestQueue.RequestFinishedListener.class);
        RequestQueue.RequestFinishedListener listener2 = mock(RequestQueue.RequestFinishedListener.class);
        RequestQueue queue = new RequestQueue(new NoCache(), mMockNetwork, 1, mDelivery);
